...
class NPSReportExport implements FromCollection,ShouldAutoSize,WithHeadings
{
    ...
    */
    public function collection()
    {
        ...
        foreach ($records as  $singleRecordComplete) {
            $report = $singleRecordComplete->member->report()->firstWhere('question_priority',5);
            $nps = (int)$singleRecordComplete->member->report()->firstWhere('question_priority',4)->answer->real_answer;
            ...
            array_push($singleRecord, isset($report->text_box_answer) ? $report->text_box_answer : "");
            array_push($singleRecord, (isset($report->classification_type) && !empty($report->classification_type->name)) ? $report->classification_type->name : "");
            array_push($singleRecord, (isset($report->campaign) && !empty($report->campaign->name)) ? $report->campaign->name : "");
            array_push($singleRecord,(isset($report->classification) && !empty($report->classification->name)) ? $report->classification->name : "");
