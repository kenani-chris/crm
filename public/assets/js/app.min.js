...
$(".missingOle").click(function() {
  var activationValue = $("input[name='disposition']:checked").attr("id");
  $("#dispo_call").val("");
  if (activationValue === "call-back") {
    $(".dispo_call").css("display", "block");
  } else {
    $(".dispo_call").css("display", "none");
  }
});

$(".q4Sales").click(function() {
  var q4ans = $("input[name='q4']:checked").val();
  $("#q4_comments").val("");
  if (q4ans === "Yes") {
    $(".q4_comments_div").css("display", "block");
  } else {
    $(".q4_comments_div").css("display", "none");
  }
});

$(".q10bs").click(function() {
  var q10ans = $("input[name='q10']:checked").val();
  $("#q10_comments").val("");
  if (q10ans === "Yes") {
    $(".q10_comments_div").css("display", "block");
  } else {
    $(".q410comments_div").css("display", "none");
  }
});

$(".q2Activate").click(function() {
  var activationValue = $("input[name='q2']:checked").val();

  //$(".notactivatedres").prop("checked", false);
  $("#callback_time").val("");
  $(".callback_time").css("display", "none");

  $(".callback-div").css("display", "none");

  if (activationValue === "No") {
    $(".callback_time").css("display", "block");
    $(".callback-div").css("display", "block");
  } else {
  }
});
...
$("body").on("click", "#followupBtn", function(event) {
  event.preventDefault();
  var id = $(this).data("id");

  $("#x_id").val(id);

  $.get("/lead/" + id + "/show/" + channel, function(data) {
    if (data.msg[0].lead) {
      $("#phoneNo").val(data.msg[0].lead.CustomerPhoneNumber);
      $(".fmessage").html(data.msg[0].lead.CustomerName);
      $(".famount").html(data.amount);
    }
  });
});
...
$(".toggleDashboard").click(function(){
    $("#dashborad-search-form").toggle('slow');
  });
