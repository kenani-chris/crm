...
<div class="card">
    <div class="card-body">
        <div class="row">
            ...
            @if(!isset($error) || isset($msg) && !empty($msg))

            <div class="col-lg-12">

<form method="post" action="{{route('phone.search',1)}}">
<div class="row">
    @csrf
...
<div class="col-sm-9">
        <div class="form-group">
            <label for="search_phone_no">Enter Phone no to search</label>
            <input type="text" id="search_phone_no" placeholder="Enter phone no to search" class="form-control rounded" name="search_phone_no" required>

            <span class="text-danger">{{ $errors->first('search_phone_no') }}</span>
        </div>
    </div>
...
<button type="submit" class="btn btn-primary btn-block pull-right btn-rounded"><i class="fa fa-search"></i> Search Phone No</button>
</div>
</div>
...
@if(!empty($customer))
            ...
            <form action="{{route('survey.intro',$customer->id)}}" id="newLeadUpload" method="POST" enctype="multipart/form-data">
            @csrf
<hr>
            <div class="form-group">
            Customer Phone No
